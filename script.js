...
function aplicarFiltros() {
  const q = normalizarBusqueda($("buscadorProductos")?.value || "");
  const max = Number($("rangoPrecio")?.value || 100000);
  const valorPrecio = $("valorPrecioFiltro");
  if (valorPrecio) valorPrecio.textContent = max >= 100000 ? "$100.000+" : money(max);

  const cards = [...document.querySelectorAll("#productos .product-card")];
  let visibles = 0;

  cards.forEach((card) => {
    const texto = normalizarBusqueda(card.dataset.nombre || "");
    const precio = Number(card.dataset.precio || 0);
    const okQ = !q || texto.includes(q);
    const okP = max >= 100000 || precio <= max;
  ...
  if (contador) contador.textContent = visibles === 1 ? "🔎 1 producto encontrado" : `🔎 ${visibles} productos encontrados`;
}
...
window.agregarAlCarrito = function (nombre, precio, imagen = "") {
  ...
  if (!nombre || !Number.isFinite(precioNumero) || precioNumero < 0) {
    mostrarToast("No se pudo agregar", "El producto no tiene un precio válido.", "error");
    return;
  }
  const existente = carrito.find((p) => p.nombre === nombre);
  if (existente) {
    existente.cantidad = limitar(Number(existente.cantidad || 0) + 1, 1, 99);
    if (!existente.imagen && imagen) existente.imagen = imagen;
  } else {
    carrito.push({ nombre: String(nombre), precio: precioNumero, imagen: String(imagen || ""), cantidad: 1 });
  }
  guardarLocalStorage("karleny_carrito", carrito);
  actualizarCarrito();
  mostrarToast("🛒 Producto agregado", `${nombre} está en tu carrito.`, "success");
};
...
function resetPersonalizador() {
  camisas = []; cantidadCamisas = 1; camisaActual = 0; ladoActual = "frente"; historyPast = []; historyFuture = []; editorColor = "#ffffff"; editorColorNombre = "Blanco";
...
window.continuarAlEditor = async function () {
  if (!camisas.length) inicializarCamisas();
  camisaActual = 0;
  ladoActual = "frente";

  // IMPORTANTE: Fabric debe calcular el tamaño cuando el editor ya es visible.
  mostrarPasoPersonalizador("editor");
  await new Promise((resolve) => requestAnimationFrame(() => requestAnimationFrame(resolve)));

  try {
    await prepararEditor();
    if (!fabricCanvas) throw new Error("No se pudo iniciar el lienzo de la camisa.");
    await cargarCamisaEnEditor(0);
    fabricCanvas.calcOffset?.();
    fabricCanvas.renderAll();
  } catch (error) {
    console.error("Error iniciando editor:", error);
    mostrarToast("No se pudo abrir el editor", "Recarga la página e inténtalo de nuevo.", "error");
    mostrarPasoPersonalizador("cantidad");
  }
};
...
function actualizarCamisaSVG(){
  const s=$("siluetaCamisa");
  if(!s)return;
  s.setAttribute("fill",editorColor);
  s.setAttribute("stroke",COLORES_CAMISA.find(c=>c.valor===editorColor)?.borde||"#d8d8dc");
}
...
async function prepararEditor(){
  ...
  const fabric = await esperarFabric();
  const canvasEl = $("canvasCamisa");
  if (!canvasEl) throw new Error("No existe #canvasCamisa en el HTML.");
  ...
  try {
    ...
    // El lienzo conserva 600×800 px internamente (30×40 cm).
    // CSS se encarga de adaptarlo al tamaño visible del móvil/PC.
    fabricCanvas.setDimensions({ width: EDITOR_WIDTH, height: EDITOR_HEIGHT });

    fabricCanvas.on("selection:created", actualizarMedidasSeleccion);
    fabricCanvas.on("selection:updated", actualizarMedidasSeleccion);
    fabricCanvas.on("selection:cleared", actualizarMedidasSeleccion);
    fabricCanvas.on("object:moving", (e) => {
      mantenerDentroDelArea(e.target);
    ...
    fabricCanvas.on("object:added", () => {
      if (!restoring) registrarHistoria();
    });
    fabricCanvas.on("object:removed", () => {
      if (!restoring) registrarHistoria();
    });

    fabricarAjusteCanvas();
    construirPaleta();
    return fabricCanvas;
  } catch (error) {
